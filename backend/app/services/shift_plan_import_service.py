...
def _lookup_workers_by_employee_code(
    session: Session,
    tenant_id: str,
    employee_codes: list[str],
) -> tuple[dict[str, uuid.UUID], list[str]]:
    """職員番号（employee_code）からワーカーUUIDへのマッピングを構築する.

    存在しない職員番号はスキップ対象としてリストで返す。
    入力値の前後空白は除去し、文字列として突き合わせを行う。

    Args:
        session: DBセッション。
        tenant_id: テナントID。
        employee_codes: 職員番号のリスト（重複あり可）。

    Returns:
        (employee_code -> worker_id マップ, 存在しない職員番号リスト)
    """
    # 前後空白を除去した上で挿入順序を保持しつつ重複排除
    unique_codes = list(dict.fromkeys(c.strip() for c in employee_codes if c.strip()))
    if not unique_codes:
        return {}, []

    workers = session.exec(
        select(Worker).where(
            Worker.employee_code.in_(unique_codes),  # type: ignore[attr-defined]
    ...
    for w in workers:
        if w.employee_code is not None:
            mapping[str(w.employee_code)] = w.id  # type: ignore[assignment]

    missing = [code for code in unique_codes if code not in mapping]
    if missing:
        logger.warning(
            "シフトインポート: %d件の職員番号に対応するワーカーが見つかりませんでした "
            "(tenant_id=%s)。詳細はレスポンスの skipped_worker_ids を参照してください。",
            len(missing),
            tenant_id,
        )
...
def import_shift_plan(
    ...
    worker_map, missing_codes = _lookup_workers_by_employee_code(
